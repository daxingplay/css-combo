...
//                    }
    ...
    // background: url("./mods/import.css");
    // list-style: url("./mods/import.css");
    // @font-face: src("./mods/import.ttf");
    modifySubImportsPath: function(content,filePath) {
        ...
        if (typeof importResult !== 'undefined' && importResult && importResult[2]) {
            ...
            // 用%号表示已经替换好的import路径，后续会再去掉百分号
            content = content.replace(importResult[2], "%" + path.relative(self.config.sourceDir,importAbsoluteUrl));
        ...
        if (typeof picAndFontResult !== 'undefined' && picAndFontResult && picAndFontResult[2]) {
            ...
            content = content.replace(picAndFontResult[2], ":" + path.relative(self.config.sourceDir,picAbsolutePath));
            return self.modifySubImportsPath(content, filePath);
        }
        return content;
    },
